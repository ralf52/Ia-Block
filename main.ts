...
// Por esta nueva versión con +++
const IA_BLOCK_REGEX = /^(\+\+\+\+|````)\s*([\s\S]*?)\s*(\+\+\+\+|````)$/;
...
export default class IABlockPlugin extends Plugin {
   private styleEl: HTMLLinkElement | null = null;
    ...
    async onload() {
         // Notificar carga del plugin
        console.log(`IA Block Generator Plugin cargado version_${this.manifest.version}`);
        ...
        this.registerMarkdownPostProcessor((el) => {
            el.findAll('p').forEach((p) => {
                // Ignorar si contiene código de triple backtick
                if (p.innerHTML.includes('```')) return;
                
                const text = p.textContent;
                const match = text?.match(IA_BLOCK_REGEX);
                
                if (match && match[2]) {  // Usar match[2] para el contenido
                    this.renderInlineIABlock(match[2], p);
                }
            });
        });
    }
     ...
     onunload() {
        // Eliminar estilos al desactivar el plugin
        if (this.styleEl) {
            document.head.removeChild(this.styleEl);
        }
        //Notificar descarga del plugin
         console.log("IA Block Generator Plugin descargado");
    ...
    private parseParams(input: string): IABlockParams {
        ...
        if (lines.length > 0) {
            ...
            while ((match = paramRegex.exec(firstLine)) !== null) {
                ...
                // ACTUALIZA ESTA LÍNEA: Añade 'g' a la lista de valores permitidos
                if (key === 'ia' && ['dp', 'cg', 'c', 'g'].includes(value)) {
                    params.ia = value as 'dp' | 'cg' | 'c' | 'g';
                }
                if (key === 'title') {
                    params.title = value;
                }
    ...
    private getModelName(ia: string): string {
        switch(ia) {
            case 'default': return 'Asistente IA';
