...
export default class IABlockPlugin extends Plugin {
    ...
    async onload() {
        ...
        // Registrar post-procesador para bloques inline
            ...
            if (lines.length > 0) {
                ...
                while ((match = paramRegex.exec(firstLine)) !== null) {
                    ...
                    if (key === 'ia' && ['default', 'dp', 'cg', 'c', 'g'].includes(value)) {
                        params.ia = value as 'default' | 'dp' | 'cg' | 'c' | 'g';
                    }
                    if (key === 'title') {
                        params.title = value;
                    }
    ...
    onunload() {
        ...
        // Limpiar todos los roots de React
    ...
    private getLogoSVG(ia: string): string {
        if (ia === 'default') {
            ...
            return `<svg height="1em" style="flex:none;line-height:1" viewBox="0 0 24 24" width="1em" xmlns="http://www.w3.org/2000/svg"><defs>
