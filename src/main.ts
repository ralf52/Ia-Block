...
export default class IABlockPlugin extends Plugin {
    ...
    try {
      ...
      if (lines.length > 0) {
        ...
        while ((match = paramRegex.exec(firstLine)) !== null) {
          ...
          if (
            key === "ia" &&
            ["default", "dp", "cg", "c", "g"].includes(value)
          ) {
  ...
  private renderIABlock(
    ...
    try {
      ...
      el.innerHTML =
        '<div class="ia-error">Error al renderizar el bloque IA</div>';
    }
  }
  ...
  private renderWithReact(container: HTMLElement, params: IABlockParams): void {
    try {
      ...
      container.innerHTML =
        '<div class="ia-error">Error al renderizar el componente</div>';
    }
  }
  ...
  private colorListener?: (e: MediaQueryListEvent) => void;

  /** Aplica las variables CSS según la configuración actual */
  public applyThemeFromSettings(): void {
    const mode = this.settings.themeMode || "auto";

    const apply = (m: "light" | "dark") => this.applyThemeVariables(m);
    ...
    if (mode === "auto") {
      ...
      if (this.colorListener)
        mq.removeEventListener("change", this.colorListener);
      this.colorListener = (e) => apply(e.matches ? "dark" : "light");
      mq.addEventListener("change", this.colorListener);
    } else {
      // Si existía un listener, eliminarlo
      try {
        const mq = window.matchMedia("(prefers-color-scheme: dark)");
        if (this.colorListener)
          mq.removeEventListener("change", this.colorListener);
  ...
  /** Inserta un conjunto reducido de variables CSS en `:root` y `body` para forzar tema en preview/print */
  private applyThemeVariables(mode: "light" | "dark"): void {
    const lightVars: Record<string, string> = {
      ...
      "--interactive-accent-hsl": "218, 100%, 50%",
    };
    ...
    try {
      ...
      // Marca el modo para referencias si hace falta
      root.setAttribute("data-iablock-theme", mode);
      body.setAttribute("data-iablock-theme", mode);
    } catch (e) {
      // en contextos donde document no exista, ignorar
    }
  }
...
class IABlockSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Si está activado, los bloques IA estarán expandidos por defecto.",
      )
      .addToggle((toggle) =>
        toggle
    ...
    // Configuración del tema (auto / claro / oscuro)
    new Setting(containerEl)
      .setName("Tema de salida")
      .setDesc(
        "Selecciona si el plugin fuerza tema claro, oscuro o sigue el sistema.",
      )
      .addDropdown((drop) =>
        drop
          .addOption("auto", "Auto (sistema)")
          .addOption("light", "Claro")
          .addOption("dark", "Oscuro")
          .setValue(this.plugin.settings.themeMode || "auto")
          .onChange(async (value) => {
            this.plugin.settings.themeMode = value as any;
            await this.plugin.saveSettings();
            this.plugin.applyThemeFromSettings();
          }),
      );
  }
}
